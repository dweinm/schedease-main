  ...
  const loadCourses = async () => {
    try {
      ...
      try {
        ...
        setInstructors(instList.map((i: any) => ({ 
          _id: String(i._id || i.id || ''), 
          name: i.name || i.userId?.name || i.user?.name || 'Unknown Instructor'
        })));
      } catch (err) {
      ...
      const mappedCourses = (response.courses || response.data || []).map((course: any) => {
        ...
        // Extract instructor ID - handle both populated objects and string IDs
        let instructorId = '';
        if (course.instructorId) {
          if (typeof course.instructorId === 'object' && course.instructorId._id) {
            instructorId = String(course.instructorId._id);
        ...
        // Extract instructor name - prioritize populated data, then denormalized field
        let instructorName = '';
        if (course.instructorId && typeof course.instructorId === 'object') {
          // First try populated instructor.userId.name
          instructorName = course.instructorId.userId?.name || 
                          course.instructorId.user?.name ||
                          course.instructorId.name || '';
        }
        // Fall back to denormalized instructorName field
        if (!instructorName && course.instructorName) {
                  ...
                  <TableRow key={course._id || course.id}>
                    ...
                    <TableCell>
                      {course.instructorName || 
                       (course.instructorId && instructors.find(i => String(i._id) === String(course.instructorId))?.name) || 
                       'Unassigned'}
                    </TableCell>
                    <TableCell>
